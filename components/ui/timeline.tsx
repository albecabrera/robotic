...
export const Timeline = ({
  ...
  return (
    <div className="w-full font-sans" ref={containerRef}>
      ...
      <div ref={ref} className="relative mx-auto max-w-6xl px-4 pb-20 md:px-6">
        {data.map((item, index) => (
          <div key={index} className="flex justify-start pt-10 md:gap-10 md:pt-32">
            <div className="sticky top-40 z-40 flex max-w-xs flex-col items-center self-start md:w-full md:flex-row lg:max-w-sm">
              <div className="absolute left-3 flex h-10 w-10 items-center justify-center rounded-full border border-border bg-background md:left-3">
                <div className="h-3 w-3 rounded-full border border-border bg-primary/20" />
              </div>
              <h3 className="hidden pl-20 text-3xl font-bold text-foreground/30 md:block md:text-5xl">
                {item.title}
              </h3>
            </div>

            <div className="relative w-full pl-20 pr-4 md:pl-4">
              <h3 className="mb-4 block text-left text-2xl font-bold text-foreground/30 md:hidden">
                {item.title}
              </h3>
              {item.content}
            </div>
